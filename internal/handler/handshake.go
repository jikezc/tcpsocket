...
func (h *ServerMsgHandler) HandleHandshakeReq(conn net.Conn, payload *message.MSG_HANDSHAKE_REQ) error {
	// 将客户端加入SessionMap
	h.Server.SessionMap[conn] = socket.Session{
		DiverId:       payload.GetDeviceId(),
		LastAliveTime: utils.GetCurrentTimestamp(),
	}
	fmt.Printf("Server当前会话: %v\n", h.Server.SessionMap)
	// 开始心跳检查
	...
	if writeErr != nil {
		...
		fmt.Printf("Server, 发送握手响应消息成功，发送 %v Bytes, respPayload: %v\n", len(pkg), respPayload)
	}
	return nil
}
